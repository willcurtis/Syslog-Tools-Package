"""
syslog-pro — Professional syslog generator (RFC3164/RFC5424 over UDP/TCP/TLS)
"""
import argparse
import ipaddress
import os
...
def build_3164(facility: int, severity: int, hostname: str, app: str, procid: str, msg: str, dt: datetime) -> bytes:
    ...
    ts = _bsd_timestamp(dt.astimezone())
...
def open_socket(transport: str, target: str, port: int, *, tls: bool, cafile: str, certfile: str, keyfile: str, insecure: bool, sni: str, bind_ip: str):
    ...
    if bind_ip:
        bind_addrinfo = socket.getaddrinfo(bind_ip, 0, family, type_)[0]
    else:
        bind_addrinfo = None
